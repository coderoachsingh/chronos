...
class DocumentLoader:
    ...
    @staticmethod
    def load_and_split(file_path: str, chunk_size: int, chunk_overlap: int) -> List[Document]:
...
class RAGEngine:
        ...
        if persist_path.exists():
            try:
                self.vector_store = FAISS.load_local(
                    self.config.PERSIST_DIR, self.embeddings, allow_dangerous_deserialization=True
                )
    ...
    def load_document(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            splits = DocumentLoader.load_and_split(file_path, self.config.CHUNK_SIZE, self.config.CHUNK_OVERLAP)
            self.vector_store.add_documents(splits)
            self.vector_store.save_local(self.config.PERSIST_DIR)
            return {"status": "success", "type": "document_loaded", "num_chunks": len(splits), "file_path": file_path}
        except Exception as e:
    ...
    def query(self, question: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "success", "type": "final_answer", "answer": result["result"], "sources": sources}
        except Exception as e:
...
app = Flask(__name__)
config = AppConfig()
engine = RAGEngine(config)  # Initialize once during app startup
...
def create_json_error(message: str, status_code: int) -> Dict[str, Any]:
...
def load_document():
    ...
    if not request.is_json:
        return create_json_error("Content-Type must be application/json", 400)

    data = request.json
    file_path = data.get("file_path")
    if not file_path:
        return create_json_error("file_path is required", 400)
...
def handle_query():
    ...
    if not question:
        return create_json_error("question is required", 400)
